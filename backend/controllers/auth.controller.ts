...
export const RegisterUser = async(req: Request, res: Response, next: NextFunction) => {
    try {
        const {fullName, email, password, confirmPassword} = req.body as IRegisterUser;
        ...
        if(!validatePassword) {
           ...
           return next(new ErrorHandler("Password should have at least 8 charactes, one uppercase letter, one lowercase letter and one special character", 400))
        }
